...
public class AlarmController {
    ...
    public ApiResponse<?> getAlarm(@RequestBody SseRequest sseRequest) {
        return ApiResponse.ofSuccess(sseService.subscribeWithUUID(sseRequest.getUuid()));
    }
    @Operation(summary = "uuid로 알람리스트 받기", description = "uuid에 맞는 알람 중 읽지않은 것을 가져온다.")
    @GetMapping("/alarms")
    public ApiResponse<?> getAlarmList(@RequestBody SseRequest sseRequest) {
        List<Integer> alarmIdList = alarmService.getAlarmList(sseRequest.getUuid());
        return ApiResponse.ofSuccess(alarmIdList);
    }
    @Operation(summary = "단일 알람 읽기", description = "알람을 하나 읽기 처리한다.")
    ...
    @Operation(summary = "단일 전체 읽기", description = "알람을 전체 다 읽기 처리한다.")
